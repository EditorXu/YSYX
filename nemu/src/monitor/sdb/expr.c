    ...
    for (i = 0; i < NR_REGEX; i ++) {
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
        ...
        switch (rules[i].token_type) {
					...
					case '-':
						tmp.type = '-';
						if((tokens[nr_token-1].type == NUM) || (tokens[nr_token-1].type == HEX) || (tokens[nr_token-1].type == REGS)) {
							tokens[nr_token] = tmp;
							nr_token++;
						}
...
static int Operator(int p, int q) {
	...
	for(int i = p; i <= q; i++) {
		if(((tokens[i].type == '+') || (tokens[i].type == '-')) && count1 == 0) {
			add_sub = i;
			count1++;
		}
		else if(((tokens[i].type == '*') || (tokens[i].type == '/') ||  (tokens[i].type == 14)) && (count1 == 0) && (count2 == 0)) {
			mul_div = i;
			count2++;
		}
		for(int j = p; j <= i - 1; j++)
		 {
			if(check_parentheses(j, i) == true) {
				if((add_sub > j) && (add_sub < i)){
					debug("j = %d, i = %d, add_sub = %d\n",j,i,add_sub);
					count1--;
				}
				else if((mul_div > j) && (mul_div < i)){
					count2--;
				}
			}
		 }
	...
	debug("add_sub = %d, mul_div = %d\n",add_sub,mul_div);
	debug("count1 = %d, count2 = %d\n",count1,count2);
